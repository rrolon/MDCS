% train neural network using complete dictionary
...
number_of_atoms = 64;
alfa = full(omp(A,X_1_667_new,[],L));; % inputs of the neural network
[~,indices] = freq_act(alfa,T_1_667_new);
main_indices = indices(1:number_of_atoms);
% A = dict; % use in case of FULL-CD and MDAS-CD methods
A = A(:,main_indices); % sub-dictionary in case of MDCS-CD method

inputs = full(omp(A,X_1_667,[],L));; % inputs of the neural network
% inputs = inputs(main_indices,:); % use in case of MDAS-CD method
